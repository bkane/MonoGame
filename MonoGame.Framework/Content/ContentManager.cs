...
namespace Microsoft.Xna.Framework.Content
{
    public partial class ContentManager : IDisposable
    {
        ...
        protected T ReadAsset<T>(string assetName, Action<IDisposable> recordDisposableObject)
        {
			...
			Console.WriteLine("Building " + assetName);
            ...
            if (Path.GetExtension(assetName).ToLower() == ".xnb")
            {
                ...
                using (Stream stream = OpenStream(assetName))
                {
                    using(BinaryReader xnbReader = new BinaryReader(stream))
                    {
						...
						Console.WriteLine("Asset is " + (compressed ? "" : "not ") + "compressed.");
                        if (compressed)
                        {
                            //decompress the xnb
                        ...
                        using(reader)
                        {
							...
							Console.WriteLine("Reader has " + reader.TypeReaders.Length + " type readers");
